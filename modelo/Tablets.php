...
class Tablets
{
    ...
    public function getDataMemorandumById($id)
    {
        $ret = BDD::getInstance()->query(
            "SELECT num_serie,M.nombre as marca,modelo,A.nombre as sector,U.nombre_apellido,U.id_usuario,D.procesador,D.memoria,D.disco,T.descripcion from system.tablets T 
...
WHERE T.id_tablet = '$id'"
    ...
    public function listarEnStock($datos_extra = "")
    {
        ...
        $inst_table = BDD::getInstance()->query(
			...
			as m from system.tablets T INNER JOIN system.tablet_desc D ON D.id_tablet_desc = T.id_tablet_desc INNER JOIN system.marcas M ON M.id_marca = D.id_marca INNER JOIN system.areas A ON A.id_area = T.id_sector INNER JOIN system.usuarios U ON U.id_usuario = T.id_usuario WHERE T.estado = 1 and A.id_area = 1"
        );
    ...
    public function dameSelectMarcas()
    {
        $table = BDD::getInstance()->query("select nombre, id_marca from system.marcas where estado = 1");
        $html_view = "<select id='select_marcas' class='form-control' name='marca'>";
        $html_view .= "<option selected='selected' value=''>Seleccionar</option>";

        while ($fila_marca = $table->_fetchRow())
        {
            $html_view .= "<option value=".$fila_marca['id_marca'].">".$fila_marca['nombre']."</option>";
        }
    ...
    public function dameSelectModelos()
    {
        $table = BDD::getInstance()->query("select id_tablet_desc,modelo from system.tablet_desc where estado = 1");
        $html_view = "<select id='select_modelos' class='form-control' name='modelo'>";
        $html_view .= "<option selected='selected' value=''>Seleccionar</option>";

        while ($fila_marca = $table->_fetchRow())
        {
            $html_view .= "<option value=".$fila_marca['id_tablet_desc'].">".$fila_marca['modelo']."</option>";
        }
