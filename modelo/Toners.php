...
class Toners
{
	...
	{
		$ret = BDD::getInstance()->query("SELECT concat(m.nombre,' ',i.modelo) AS impresora,a.nombre AS area,t.cantidad FROM system.toners t 
			...
			WHERE a.estado = 1 AND i.estado = 1 AND m.estado = 1 AND t.estado = 1 AND t.id_toner = '$id'")->_fetchRow();
		return $ret; 
	}
	...
	{
		return BDD::getInstance()->query("SELECT t.id_area FROM system.toners t
			WHERE t.id_toner = '$id'")->_fetchRow()['id_area'];
	}
