...
<form id="form_asignar_disco_a_compu" autocomplete="off">
<fieldset>
<legend>Complete los Datos <small>(hay validaciones al crear)</small></legend>
        ...
        <!-- 
        Consigo los discos que no estan asignados no hay restricciones porque no se cuentan satas, igualmente preguntar.

        SELECT * FROM system.discos WHERE
        EXISTS (SELECT id_pk_producto FROM system.vinculos WHERE id_tipo_producto=3)
        AND estado = 1;
        -->
...
<script type="text/javascript">
    ...
    $('#form_asignar_disco_a_compu #select_unidades_discos option[value='+3+']').attr('selected', 'selected');

    $("#form_asignar_disco_a_compu").validate({
        onfocusout: false,
        onkeyup: false,
        onclick: false,
        onsubmit: true,
        ...
        submitHandler : function (form) {
            ...
            console.log($("#form_asignar_disco_a_compu").serialize());
            var dataUrl = $("#form_asignar_disco_a_compu").serialize() + "&tipo=Disco&id_cpu="+$("#tab_memorias_y_discos").attr("id_cpu");
